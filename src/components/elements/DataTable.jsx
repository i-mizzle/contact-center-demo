...
const DataTable = ({
    ...
    const renderCellContent = (header, data) => {
        const value = data[header.column]

        if (isValidElement(value)) {
            return value
        }

        if (header.columnDataType === 'image') {
            return <img src={value} alt="" />
        }

        if (header.columnDataType === 'link') {
            return <Link to={value} alt="" className="text-ink-navy font-medium"> {value} </Link>
        }

        if (header.columnDataType === 'popoverTrigger') {
    ...
    return (
        <Fragment>
            ...
            {!tableData || tableData.length === 0 ? 
                ...
                <div className="pt-2">
                    {/* table header */}
                    <ul className="bg-gray-50 dark:bg-transparent flex flex-row justify-between items-center w-full text-xs mt-1 px-3 py-2 relative font-semibold">
                       {/* <li className="w-1/12" />  */}
                        ...
                        {tableHeaders.map((header, headerIndex) => (
                            ...
                            <li className={`${columnWidths[header.column]} flex flex-row items-center uppercase justify-between ml-2 text-gray-600 dark:text-gray-300 text-xs`} key={headerIndex} >
                        ...
                        <span className="absolute right-1">
                            <Popover className="relative">
                                ...
                                >
                                    <div className="bg-white dark:bg-stone-900 text-center z-999 p-4 shadow-md rounded mt-3">
                                        {/* {allFields.map((field, fieldIndex) => )} */}
                                        <p className="font-medium text-gray-400 text-sm pb-2 mb-2 border-b border-gray-200 dark:bg-stone-900 text-center">All Fields</p>
                    ...
                    {allItems.map((data, dataIndex) => (
                        <ul onClick={()=>{performRowAction(dataIndex)}} className={`flex flex-row items-start w-full bg-opacity-40 text-xs mt-3 hover:bg-gray-50 dark:hover:bg-black/20 ${tableOptions.clickableRows ? 'cursor-pointer' : 'cursor-default'} transition duration-200 py-2 px-1 font-sofia-pro text-gray-500 dark:text-gray-200 relative ${data.selected ? 'bg-gray-300/10 dark:bg-black/30' : ''}`} key={dataIndex}>
